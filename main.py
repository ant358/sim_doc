...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filename", type=str, default="data/rent_rome_text.csv"
    )
    ...
    end_time = time.time()
    print(
        "Signature Matrix computed in:\t %.2f seconds."
        % (end_time - start_time)
    )
    ...
    dataset_reduced = dataset.drop_duplicates(
        subset=["similar_docs"], keep="first"
    )
    # output the reduced dataset to a file
    dataset_reduced.to_csv(
        f"{OUTPUT}_reduced.csv",
        index=False
    )
